(function () {
  ...
  function render() {
    ...
    document.getElementById("buildings-shop").innerHTML = "<p class=\"income-timer-hint\">Buildings produce resources & new day every " + G.INCOME_INTERVAL_SEC + " sec. Real estate rent is collected every 7 days.</p>" + shopCategories.map(({ category, entries }) => {
      ...
      return "<div class=\"category-block " + (open ? "" : "collapsed") + "\"><div class=\"category-header\" data-category=\"" + escapeHtml(category) + "\" data-tab=\"" + tabShop + "\" role=\"button\" tabindex=\"0\"><span class=\"badge category-toggle-badge\" title=\"" + (open ? "Close" : "Open") + "\"><span class=\"category-toggle-text\">" + (open ? "Close" : "Open") + "</span></span><span class=\"category-title\">" + escapeHtml(category) + "</span></div><div class=\"category-content\">" + entries.map(([type, def]) => {
    ...
    if (achievementsList) {
      ...
      achievementsList.innerHTML = ACH_CAT_ORDER.filter((cat) => byCategory[cat] && byCategory[cat].length > 0).map((category) => {
        ...
        return "<div class=\"category-block " + (open ? "" : "collapsed") + "\"><div class=\"category-header\" data-category=\"" + escapeHtml(category) + "\" data-tab=\"" + tabAch + "\" role=\"button\" tabindex=\"0\"><span class=\"badge category-toggle-badge\" title=\"" + (open ? "Close" : "Open") + "\"><span class=\"category-toggle-text\">" + (open ? "Close" : "Open") + "</span></span><span class=\"category-title\">" + escapeHtml(category) + "</span><span class=\"category-count\">" + unlockedInCat.length + " / " + list.length + "</span></div><div class=\"category-content\">" + content + "</div></div>";
      }).join("");
    }
